...
public sealed class UpgradeCommand
{
    ...
    public Command Build()
    {
        ...
        var cmd = new Command("upgrade", "Check for updates and upgrade Krnl-AI CLI") { checkOnly, versionOpt };
        ...
        cmd.SetAction(async (ParseResult r, CancellationToken ct) =>
        {
            ...
            if (latestVersion == currentVersion) { AnsiConsole.MarkupLine("[green]You are already using the latest version![/]"); return 0; }
            if (check) { AnsiConsole.MarkupLine($"[yellow]Update available:[/] {currentVersion} → [green]{latestVersion}[/]"); AnsiConsole.MarkupLine("Run [bold]krnlai upgrade[/] to install."); return 0; }

            var targetVersion = version ?? latestVersion;
            if (!AnsiConsole.Confirm($"Install version {targetVersion}?")) { AnsiConsole.MarkupLine("[yellow]Upgrade cancelled.[/]"); return 0; }
    ...
    private static async Task<string?> DownloadPackageAsync(string version, CancellationToken ct)
    {
        try
        {
    ...
    private static bool VerifyChecksum(string packagePath, string version)
    {
        try
        {
            ...
            AnsiConsole.MarkupLine($"  Verification: [green]pass (prefix: {hashString[..8]})[/]");
            return true;
        }
        catch { return false; }
    }

    private static async Task<bool> InstallVersionAsync(string version, string previousVersion, CancellationToken ct)
    {
        try
        {
    ...
    private static async Task<bool> RollbackAsync(string previousVersion, CancellationToken ct)
    {
        try
        {
    ...
    private void LogVersion(string version, string status)
    {
        try
        {
            ...
            {
                try { var existing = File.ReadAllText(_versionLogPath); log = JsonSerializer.Deserialize<List<VersionLogEntry>>(existing) ?? []; }
                catch { log = []; }
            }
            log.Add(new VersionLogEntry { Version = version, Status = status, Timestamp = DateTimeOffset.UtcNow.ToString("O") });
            File.WriteAllText(_versionLogPath, JsonSerializer.Serialize(log, new JsonSerializerOptions { WriteIndented = true }));
        }
        catch { }
    }

    private sealed class VersionLogEntry { public string Version { get; set; } = ""; public string Status { get; set; } = ""; public string Timestamp { get; set; } = ""; }
}
