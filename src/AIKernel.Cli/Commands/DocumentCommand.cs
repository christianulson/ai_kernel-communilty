...
public sealed class DocumentCommand(CliContext ctx, ConsoleRenderer renderer)
{
    ...
    {
        var limitOpt = new Option<int>(""--limit"", () => 50, ""Max results"");
    ...
    private Command BuildStatus()
    {
        ...
        cmd.SetAction(async (ParseResult r, CancellationToken ct) =>
        {
            ...
            renderer.Console.MarkupLine($""[bold]Document ID:[/] {doc.GetProperty(""documentId"").GetString()}""); renderer.Console.MarkupLine($""[bold]File:[/] {doc.GetProperty(""fileName"").GetString()}""); renderer.Console.MarkupLine($""[bold]Size:[/] {doc.GetProperty(""fileSize"").GetInt64():N0} bytes""); renderer.Console.MarkupLine($""[bold]Format:[/] {doc.GetProperty(""format"").GetString()}""); renderer.Console.MarkupLine($""[bold]Status:[/] {doc.GetProperty(""status"").GetString()}"");
            var err = doc.TryGetProperty(""errorMessage"", out var errEl) && errEl.ValueKind == JsonValueKind.String ? errEl.GetString() : null;
            renderer.Console.MarkupLine($""[bold]Error:[/] {err ?? ""none""}""); renderer.Console.MarkupLine($""[bold]Chunks:[/] {doc.GetProperty(""chunkCount"").GetInt32()}""); renderer.Console.MarkupLine($""[bold]Created:[/] {doc.GetProperty(""createdAt"").GetString()}""); 
            if (doc.TryGetProperty(""completedAt"", out var cmpEl) && cmpEl.ValueKind == JsonValueKind.String) renderer.Console.MarkupLine($""[bold]Completed:[/] {cmpEl.GetString()}"");
            return 0;
        });
        return cmd;
    }
    ...
    private Command BuildSearch()
    {
        ...
        var topKOpt = new Option<int>(""--top-k"", () => 10, ""Max results"");
    ...
    private Command BuildUpload()
    {
        ...
        cmd.SetAction(async (ParseResult r, CancellationToken ct) =>
        {
            ...
            if (!resp.IsSuccessStatusCode) { renderer.Console.MarkupLine(""[red]Upload failed (HTTP {0})[/]"", (int)resp.StatusCode); return 1; }
            var result = await resp.Content.ReadFromJsonAsync<JsonElement>(cancellationToken: ct);
