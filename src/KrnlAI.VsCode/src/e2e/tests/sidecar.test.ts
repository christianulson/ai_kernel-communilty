...
suite('Sidecar', () => {
    ...
    test('Extension_Activate_ShouldRegisterCommands', async () => {
        const commands = await import('vscode').then((vs) => vs.commands.getCommands(true));
        const krnlaiCommands = commands.filter((c: string) => c.startsWith('krnlai.'));
        assert.ok(
            krnlaiCommands.length >= 8,
