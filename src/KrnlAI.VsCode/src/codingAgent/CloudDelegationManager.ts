...
export class CloudDelegationManager {
    ...
    async delegate(task: string, context: EditorContext): Promise<DelegationResult> {
        ...
        return withProgress('Delegating to Krnl-AI Cloud...', async (progress, token) => {
            ...
            try {
                ...
                while (attempts < maxAttempts) {
                    ...
                    const statusResponse = await fetch(`${this.endpoint}/api/delegate/${delegateId}/status`, {
    ...
    listDelegations(): DelegationResult[] {
        return Array.from(this._pending.values()).sort((a, b) => b.createdAt - a.createdAt);
    }
