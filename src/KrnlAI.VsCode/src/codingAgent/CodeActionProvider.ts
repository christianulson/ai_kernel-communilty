...
export class CodeActionProvider implements vscode.CodeActionProvider {
    provideCodeActions(
        ...
        if (context.diagnostics.length > 0) {
            const fix = new vscode.CodeAction('Fix with Krnl-AI', vscode.CodeActionKind.QuickFix);
            fix.command = {
        ...
        if (hasSelection) {
            const explain = new vscode.CodeAction('Explain with Krnl-AI', vscode.CodeActionKind.Refactor);
            explain.command = {
        ...
        if (/class\s+\w+|function\s+\w+/.test(text)) {
            const test = new vscode.CodeAction('Generate Test with Krnl-AI', vscode.CodeActionKind.Refactor);
            test.command = {
