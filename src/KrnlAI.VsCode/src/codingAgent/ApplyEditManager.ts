...
export class ApplyEditManager {
    async applyChange(change: FileChange, approvalGate?: ApprovalGate): Promise<boolean> {
    ...
    async applyMultiFile(changes: FileChange[], label: string, approvalGate?: ApprovalGate): Promise<boolean> {
    ...
    private async _requestApproval(change: FileChange, approvalGate?: ApprovalGate): Promise<boolean> {
        if (!approvalGate) return true;

        const decision = await approvalGate.requestApproval(`Modificar: ${change.label}`, [
    ...
    private async _applyEdit(uri: vscode.Uri, originalContent: string, newContent: string): Promise<boolean> {
        try {
    ...
    async applyWithDiff(change: FileChange, approvalGate?: ApprovalGate): Promise<boolean> {
        await this.showDiff(change.filePath, change.originalContent, change.newContent, change.label);

        return this.applyChange(change, approvalGate);
    }

    async applyMultiFileWithDiff(changes: FileChange[], label: string, approvalGate?: ApprovalGate): Promise<boolean> {
        if (changes.length === 1) {
