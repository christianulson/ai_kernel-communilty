...
public sealed class MemoryAndEpisodesEndpointTests(SidecarWebAppFactory factory) : IClassFixture<SidecarWebAppFactory>
{
    ...
    {
        var res = await _http.GetAsync("/memory/metrics", TestContext.Current.CancellationToken).ConfigureAwait(false);
    ...
    {
        var res = await _http.GetAsync("/episodes/search", TestContext.Current.CancellationToken).ConfigureAwait(false);
    ...
    {
        var res = await _http.GetAsync("/episodes/test-123", TestContext.Current.CancellationToken).ConfigureAwait(false);
